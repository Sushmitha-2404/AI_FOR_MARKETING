...
print("Together Key:", TOGETHER_API_KEY)
...
def together_response(prompt):
    ...
    res_json = res.json()
    print(res_json)  # <-- this will help you debug what’s coming back

    if "choices" in res_json:
        return res_json["choices"][0]["message"]["content"]
    elif "error" in res_json:
        return f"[TOGETHER ERROR] {res_json['error']['message']}"
    else:
        return "[TOGETHER ERROR] Unexpected response format"
...
def groq_response(prompt):
    ...
    res = requests.post(url, headers=headers, json=data)
    res_json = res.json()
    print(res_json)  # Debug the response
